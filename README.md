...
__Note:__ Using this example registry is only necessary to make these exercises standalone, if you have a personal oci registry available, that can be used too.  
...
Identify the appropriate blob sha from the manifest output from previous cmd:
...
typically relies on UUIDs for storing documents, however these can be taxing to use as an ID when performing operations. The `alias` command  
...
We're going to use the cdx SBOM fetched in the first [exercise](#step-1-fetch). If you haven't already, go back and fetch it.
...
be handled together, or at least linked in a way that makes it easy to know what goes with what. The `tag` command allows a user to specifcy a 'tag'  
...
We're going to use the cdx SBOM fetched in the first [exercise](#step-1-fetch). If you haven't already, go back and fetch it.  
Feel free to use any already in your Database instead, just replace the SBOM ID in the command with your chosen SBOM ID.

Since this SBOM originated from a bomctl container image, lets add those tags. Feel free to add more, space separated.
...
Ok, it's probably redundant to have this SBOM tagged with `bomctl`, since thats all we've been handling, lets remove it.
...
Ok this particular SBOM still has too many superfluous tags, so lets just clear them all.
